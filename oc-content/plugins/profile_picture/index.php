...
function profile_picture_upload(){
    ...
    echo '<h2><div class="icon-my-photo"></div><span>Upload your photo or logo</span></h2>';
   ...
   // Configuration - Your Options ///////////////////////////////////////////////////////
	...
	if(!in_array($ext,$allowed_filetypes))
	    die('<div class="empty">The file you attempted to upload is not allowed.</div>');
 
	// Now check the filesize, if it is too large then DIE and inform the user.
	if(filesize($_FILES['userfile']['tmp_name']) > $max_filesize)
	    die('<div class="empty">The file you attempted to upload is too large.</div>');
 
	// Check if we can upload to the specified path, if not DIE and inform the user.
	if(!is_writable($upload_path))
	{
	    die('<div class="empty">You cannot upload to the specified directory, please CHMOD it to 777.</div>');
	}
	...
	if(move_uploaded_file($_FILES['userfile']['tmp_name'],$upload_path . 'profile'.$user_id.$ext)){
	    ...
	    echo '<div class="empty">There was an error during the file upload.  Please try again.</div>'; // It failed :(.
	}
     }
    ...
    echo '<div class="empty">Upload picture with maximum width of 250px and maximum height of 200px.';
    echo '</div></div>';
...
function profile_picture_show( $maxwidth = NULL, $what = NULL, $maxheight = NULL ){
   ...
   // Configuration - Your Options ///////////////////////////////////////////////////////
    ...
    $maxwidth = isset($maxwidth) ? $maxwidth : 300;

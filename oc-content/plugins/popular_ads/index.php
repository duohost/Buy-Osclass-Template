...
function popular_ads_start() {
	...
	foreach($results as $result){
     $view_count[$result['fk_i_item_id']] = isset($view_count[$result['fk_i_item_id']]) ? $view_count[$result['fk_i_item_id']] : 0;
	...
	foreach($view_count as $item_id=>$views)
	{
		...
		// get description
	        $desc=$conn->osc_dbFetchResult('SELECT fk_c_locale_code, s_title, s_description FROM %st_item_description WHERE fk_i_item_id = %d AND fk_c_locale_code = "%s"', DB_TABLE_PREFIX, $item_id, osc_current_user_locale()); //Get active status of item
		...
		$item_array[] =   array('fk_i_user_id'=>$result['fk_i_user_id'],
					...
					'dt_pub_date'=>(isset($result['dt_pub_date']) ? $result['dt_pub_date'] : ''),
					'dt_mod_date'=>(isset($result['dt_mode_date']) ? $result['dt_mode_date'] : ''),
					...
					'locale'=>array('en_US'=>array('fk_i_item_id'=>$item_id,
									...
									's_title'=>$desc['s_title'],
									's_description'=>$desc['s_description'],
									's_what'=>(isset($desc['s_what']) ? $desc['s_what'] : '')
									)
							)				
					);
